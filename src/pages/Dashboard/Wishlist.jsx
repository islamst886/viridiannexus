      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-2">Saved properties you are keeping an eye on.</p>
          </div>
          <span className="bg-brand-primary/10 text-brand-dark py-1 px-4 rounded-full text-sm font-bold">
        ...
        {wishlist.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-16 text-center">
            ...
            <p className="text-gray-500 mb-6">Browse our properties and click the heart icon to save them here.</p>
            <Link to="/projects" className="inline-block bg-brand-primary text-white px-6 py-3 rounded-lg font-bold hover:bg-brand-dark transition-colors">Browse Projects</Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {wishlist.map((property) => (
              <div key={property.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden group">
                ...
                <div className="p-6">
                  ...
                  <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-100">
                    <span className="text-lg font-bold text-brand-dark">{property.price}</span>
                    <Link to={`/property/${property.propertyId || property.id}`} className="text-sm font-bold text-brand-primary hover:underline transition-colors">View Details →</Link>
                  </div>
                </div>
              </div>
            ))}
